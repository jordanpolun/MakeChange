...
extension UIImage {
    ...
    private func sumRGB(color: UIColor) -> CGFloat {
        var red: CGFloat = 0
        var green: CGFloat = 0
        var blue: CGFloat = 0
        var alpha: CGFloat = 0
    ...
    func getLightestColor() -> UIColor {
        let colors = self.getColors()
        var lightest = colors?.background
        if (sumRGB(color: (colors?.primary)!) > sumRGB(color: (lightest)!)) {
            if (sumRGB(color: (colors?.primary)!) != sumRGB(color: UIColor.white))
            {
                lightest = colors?.primary
            }
        }
        if (sumRGB(color: (colors?.secondary)!) > sumRGB(color: (lightest)!)) {
            if (sumRGB(color: (colors?.secondary)!) != sumRGB(color: UIColor.white))
            {
                lightest = colors?.secondary
            }
        }
        if (sumRGB(color: (colors?.detail)!) > sumRGB(color: (lightest)!)) {
            if (sumRGB(color: (colors?.detail)!) != sumRGB(color: UIColor.white))
            {
                lightest = colors?.detail
            }
        }
        return lightest!
    }
